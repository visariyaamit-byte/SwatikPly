...
// Get all grades (for MDF, Flexi products — Board now uses companies)
...
// Get Board companies (for reports)
export async function getBoardCompanies() {
  const supabase = await createClient()

  const { data, error } = await supabase
    .from('companies')
    .select('id, name')
    .eq('product_type', 'Board')
    .order('name')
...
// Get products by company (for Plywood and Board)

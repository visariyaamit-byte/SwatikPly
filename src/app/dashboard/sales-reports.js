...
export default function SalesReports({ productTypes, companies, boardGrades, boardCompanies }) {
  ...
  useEffect(() => {
    ...
    if (selectedType === 'Plywood') {
      ...
      setAvailableCategories(boardCompanies)
    } else if (['MDF', 'Flexi'].includes(selectedType)) {
      loadGradesForType()
    } else {
  ...
  async function loadSalesForCategory() {
    ...
    if (selectedType === 'Plywood' || selectedType === 'Board') {
  ...
  const isCompanyBased = selectedType === 'Plywood' || selectedType === 'Board'
  const isGradeBased = ['MDF', 'Flexi'].includes(selectedType)
  const secondDropdownLabel = isCompanyBased
    ? 'Select Company'
    : isGradeBased
    ...
    <div className="bg-white border border-neutral-200 rounded-2xl p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div>
          ...
          >
            <option value="">-- Choose {isCompanyBased ? 'Company' : 'Type'} --</option>

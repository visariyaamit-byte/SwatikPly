...
export default function AddInventoryForm({ companies, boardCompanies }) {
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [productType, setProductType] = useState('Plywood')
  const router = useRouter()

  const usesCompany = productType === 'Plywood' || productType === 'Board'
  ...
  const getCompanyList = () => {
    if (productType === 'Plywood') return companies || []
    if (productType === 'Board') return boardCompanies || []
    return []
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {/* Company (for Plywood and Board) or Grade/Color/Type (for MDF/Flexi) */}
        {usesCompany ? (
          <div>

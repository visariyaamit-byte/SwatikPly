...
export default function NewBoardCompanyPage() {
  return (
    <div>
      {/* Header */}
      <div className="mb-8">
        <Link
          href="/dashboard/inventory/board-companies"
          className="inline-flex items-center gap-2 text-neutral-600 hover:text-neutral-900 mb-4 transition-colors"
        >
          <ArrowLeft size={20} />
          Back to Board Companies
        </Link>

        <h1 className="text-3xl font-bold tracking-tight text-brand">Add New Board Company</h1>
        <p className="text-neutral-600 mt-1">Add a board manufacturer/brand</p>
      </div>

      {/* Form */}
      <div className="bg-white border border-neutral-200 rounded-2xl p-8 max-w-2xl">
        <form action={createCompany} className="space-y-6">
          <input type="hidden" name="product_type" value="Board" />
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g., National, Kitply, etc."
            />
          </div>

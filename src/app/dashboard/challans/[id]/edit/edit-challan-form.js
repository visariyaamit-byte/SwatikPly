...
import { updateChallan } from '@/app/actions/challans'
import { getSitesByCustomer } from '@/app/actions/sites'
import { useRouter } from 'next/navigation'
import { Plus, Trash2, Package } from 'lucide-react'

export default function EditChallanForm({ challan, inventory, customers, initialSites }) {
  const router = useRouter()
  const [isSubmitting, setIsSubmitting] = useState(false)

  // Form fields — pre-populated from challan
  const [selectedCustomerId, setSelectedCustomerId] = useState(challan.customer_id || '')
  const [customerName, setCustomerName] = useState(challan.customer_name || '')

  const [sites, setSites] = useState(initialSites || [])
  const [selectedSiteId, setSelectedSiteId] = useState(challan.site_id || '')
  const [siteAddress, setSiteAddress] = useState(challan.site_address || '')

  const [phone, setPhone] = useState(challan.phone || '')
  const [additionalPhone, setAdditionalPhone] = useState(challan.additional_phone || '')

  const [date, setDate] = useState(challan.date || new Date().toISOString().split('T')[0])

  // Initialize items from existing challan items
    ...
    challan.challan_items.map((item, index) => ({
      ...
      maxQuantity: item.inventory_id
        ? (() => {
            ...
            // Available = current stock + what this challan already deducted
            return inv ? inv.quantity + item.quantity : item.quantity
          })()
        : null
    }))
  )
  ...
  function addInventoryItem(inventoryId) {
    ...
    if (inventoryItem.product_type === 'Plywood') {
      ...
      description = `${inventoryItem.company?.name} Board ${inventoryItem.measurement} ${inventoryItem.thickness}`
  ...
  async function handleSubmit(e) {
    ...
    const result = await updateChallan(challan.id, formData)

    if (result.error) {
      alert('Error: ' + result.error)
      setIsSubmitting(false)
    } else {
      router.push(`/dashboard/challans/${challan.id}`)
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white border border-neutral-200 rounded-2xl p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        {/* Site Address (manual or auto-filled) */}
        <div>
          <label className="block text-sm font-medium text-neutral-700 mb-2">
            Site Address
          </label>
          <input
            ...
            placeholder="Auto-filled from site or type manually"
            className="w-full px-4 py-3 bg-neutral-50 border border-neutral-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>
          ...
          >
            ...
            {inventory.map(item => {
              ...
              if (item.product_type === 'Plywood') {
                ...
                label = `${item.company?.name} Board ${item.measurement} ${item.thickness} (Stock: ${item.quantity})`
